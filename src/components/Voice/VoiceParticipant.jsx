...
const VoiceParticipant = ({ participant }) => {
    return (
        <div className="flex items-center gap-2 rounded px-2 py-0.5 mx-2 ml-7">
            ...
            <span className="flex-1 truncate text-[13px] text-gray-400">
                {participant.name}
            </span>

            {participant.isMuted && (
                <MicrophoneSlash className="size-3.5 shrink-0 text-gray-500" />
            )}
        </div>
    );
};
