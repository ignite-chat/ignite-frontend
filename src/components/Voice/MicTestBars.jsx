 ...
 *
 * @param {{ deviceId?: string | null, outputDeviceId?: string | null }} props
 */
const MicTestBars = ({ deviceId, outputDeviceId }) => {
  ...
  useEffect(() => {
    ...
    const start = async () => {
      try {
        // Always create our own stream for the test — this is independent from
        // the LiveKit room's mic track, so it won't steal the mic or cause muting.
        ...
        // Resume the AudioContext — browsers may suspend it due to autoplay policies,
        // which would silently block loopback audio playback.
        ...
        const tick = () => {
          ...
          // Sample frequency bins into bars, weighted toward lower frequencies (voice range)
          const bars = [];
          for (let i = 0; i < BAR_COUNT; i++) {
            // Use first ~70% of bins (voice-relevant frequencies)
